...
function randomEllipsePos(edgeInset, centerMin) {
  for (let i = 0; i < 80; i++) {
    ...
    if (Math.hypot(x - CENTER.x, y - CENTER.y) > centerMin && isInsideEllipse(x, y)) {
      return { x, y };
    }
  }
  return { x: ARENA.cx - ARENA.rx * 0.4, y: ARENA.cy };
}
...
function resetGame() {
  ...
  state.levelBonus = 0; state.messageTimer = 0;
  fx = defaultFx();
...
function resetPlayer() {
  ...
  player.vx = 0; player.vy = 0; player.r = 16;
  player.invincible = 2;
}

function resetHunter() {
  const pos = randomEllipsePos(0.15, 200);
  // Ensure hunter spawns far from player
  for (let i = 0; i < 20; i++) {
    const p = randomEllipsePos(0.15, 200);
    if (Math.hypot(p.x - player.x, p.y - player.y) > 300) {
      hunter.x = p.x; hunter.y = p.y;
      hunter.vx = 0; hunter.vy = 0; hunter.r = 15;
      return;
    }
  }
  hunter.x = pos.x; hunter.y = pos.y;
  hunter.vx = 0; hunter.vy = 0; hunter.r = 15;
}
...
function updatePlayer(dt) {
  ...
  // Angular momentum / orbital assist
...
function updateHunter(dt) {
  ...
  // Elliptical boundary for hunter
...
function render() {
  ...
  // Shift origin so ellipse center is at (ARENA.cx, ARENA.cy) in world coords
  // but we draw relative to top-left of bounding box
...
function drawArena() {
  // Draw elliptical arena background
  ctx.save();
...
function activateCheat(rawCode) {
  ...
  switch (code) {
    ...
    case "skip":
      ...
      return;
    case "warpone":
      state.level = 1; startLevel("Warped to level 1"); return;
    case "warpfive":
      state.level = 5; startLevel("Warped to level 5"); return;
    case "warpten":
      state.level = 10; startLevel("Warped to level 10"); return;
    case "warpfifteen":
      state.level = 15; startLevel("Warped to level 15"); return;
    case "warptwenty":
      state.level = 20; startLevel("Warped to level 20"); return;
    case "warpthirty":
      state.level = 30; startLevel("Warped to level 30"); return;
    case "warphundred":
      state.level = 100; startLevel("Warped to level 100"); return;
    case "life":
      state.lives++;
      showMessage("+1 life");
      break;
    ...
    case "tax":
      ...
      } else { showMessage("Need 100 score"); }
      break;
    case "medic":
      state.lives++;
      resetPlayer();
    ...
    case "orbplus":
      if (!addProgress(1)) showMessage("+1 orb progress");
      break;
    case "orbstorm":
      if (!addProgress(3)) showMessage("+3 orb progress");
      break;
    case "orbquake":
      if (!addProgress(6)) showMessage("+6 orb progress");
      break;
    case "magnet":
      fx.magnet = 8;
    ...
    case "collect": {
      ...
      if (res === null) showMessage("No orb");
      else if (!res) showMessage("Collected nearest orb");
      break;
    }
    case "shuffleorbs":
      shuffleOrbs();
    ...
    case "heal":
      ...
      break;
    case "home": {
      const hp = randomEllipsePos(0.3, 250);
    ...
    case "cocoon":
      ...
      break;
    case "chaos":
      activateCheat(POSITIVE_POOL[Math.floor(Math.random() * POSITIVE_POOL.length)]);
      return;
    case "mystery": {
      const pool = CHEAT_CODES.filter(c => c !== "mystery" && c !== "madness" && c !== "omega");
    ...
    case "omega":
      ...
      return;
    case "madness": {
      const pool = CHEAT_CODES.filter(c => c !== "madness" && c !== "omega" && c !== "mystery");
      for (let i = 0; i < 5; i++) {
        activateCheat(pool[Math.floor(Math.random() * pool.length)]);
      }
      return;
    ...
    case "godmode":
      fx.invincible = Math.max(fx.invincible, 12);
      fx.turbo = Math.max(fx.turbo, 8);
      fx.supermagnet = Math.max(fx.supermagnet, 4);
      fx.goldrush = Math.max(fx.goldrush, 12);
      fx.double = Math.max(fx.double, 10);
      fx.lucky = Math.max(fx.lucky, 10);
      fx.orbitBoost = Math.max(fx.orbitBoost, 8);
      fx.feather = Math.max(fx.feather, 10);
      state.levelBonus += 150;
      state.levelTarget = Math.max(1, state.levelTarget - 2);
      fx.shieldHit = true;
      fx.interest = Math.max(fx.interest, 10);
      fx.payday = Math.max(fx.payday, 3);
      addScore(150);
      completeLevel();
      showMessage("GODMODE ACTIVATED");
      return;
  }
  updateHUD();
}
